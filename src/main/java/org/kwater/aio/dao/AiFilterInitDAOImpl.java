...
public class AiFilterInitDAOImpl implements IAiProcessInitDAO
{
    ...
    {
        String strQuery = "SELECT * FROM ai_filter_init";
    ...
    {
        String strQuery = "SELECT * FROM ai_filter_init WHERE item=?";
        try
        {
    ...
    {
        String strQuery = "UPDATE ai_filter_init SET value=? WHERE item='f_operation_mode'";
    ...
    {
        String strQuery = "UPDATE ai_filter_init SET value=? WHERE item=?";
        return jdbcTemplate.update(strQuery, value, item);
    }

    public int updateTi(float value)
    {
        String strQuery = "UPDATE ai_filter_init SET VALUE = ? WHERE item REGEXP 'AIF-20.'";
