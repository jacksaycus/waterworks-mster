...
public class AiPeriDisinfectionControlDAOImpl implements IAiProcessControlDAO
{
    ...
    {
        String strQuery = "SELECT * FROM ai_peri_disinfection_control WHERE update_time=? and run_time=? and name=? and kafka_flag=?";
    ...
    public List<AiProcessControlDTO> select(Date run_time, int kafka_flag)
    {
        String strQuery = "SELECT * FROM " +
                "(SELECT * FROM ai_peri_disinfection_control WHERE run_time>=? and kafka_flag=? ORDER BY run_time DESC LIMIT 100) " +
                "ai_peri_disinfection_control GROUP BY name";
    ...
    {
        String strQuery = "UPDATE ai_peri_disinfection_control SET kafka_flag=? WHERE update_time=? and run_time=? and name=?";
    ...
    {
        String strQuery = "DELETE FROM ai_peri_disinfection_control WHERE update_time < ?";
