...
@Repository
public class AiReceivingAlarmDAOImpl implements IAiProcessAlarmDAO
{
    @Autowired
    ...
    {
        String strQuery = "insert into ai_receiving_alarm values (?, ?, ?)";
    ...
    {
        String strQuery = "SELECT * FROM ai_receiving_alarm WHERE alarm_time>=? AND kafka_flag=? ORDER BY alarm_time DESC";
    ...
    {
        String strQuery = "UPDATE ai_receiving_alarm SET kafka_flag=? WHERE alarm_id=? AND alarm_time=?";
    ...
    {
        String strQuery = "DELETE FROM ai_receiving_alarm WHERE alarm_time<?";
