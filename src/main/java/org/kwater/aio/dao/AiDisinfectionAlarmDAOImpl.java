...
@Repository
public class AiDisinfectionAlarmDAOImpl implements IAiProcessAlarmDAO
{
    @Autowired
    ...
    {
        String strQuery = "insert into ai_disinfection_alarm values (?, ?, ?)";
    ...
    {
        String strQuery = "SELECT * FROM ai_disinfection_alarm WHERE alarm_time>=? AND kafka_flag=? ORDER BY alarm_time DESC";
    ...
    {
        String strQuery = "UPDATE ai_disinfection_alarm SET kafka_flag=? WHERE alarm_id=? AND alarm_time=?";
    ...
    {
        String strQuery = "DELETE FROM ai_disinfection_alarm WHERE alarm_time<?";
