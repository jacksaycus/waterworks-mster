...
public class AiSedimentationControlDAOImpl implements IAiProcessControlDAO
{
    ...
    {
        String strQuery = "SELECT * FROM ai_sedimentation_control WHERE update_time=? AND run_time=? AND name=? AND kafka_flag=?";
        try
        {
    ...
    public List<AiProcessControlDTO> select(Date run_time, int kafka_flag)
    {
        String strQuery = "SELECT * FROM " +
                "(SELECT * FROM ai_sedimentation_control WHERE run_time>=? AND kafka_flag=? ORDER BY run_time DESC LIMIT 100) " +
                "ai_sedimentation_control GROUP BY name";
    ...
    {
        String strQuery = "UPDATE ai_sedimentation_control SET kafka_flag=? WHERE update_time=? AND run_time=? AND name=?";
    ...
    {
        String strQuery = "DELETE FROM ai_sedimentation_control WHERE update_time < ?";
