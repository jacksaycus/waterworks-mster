...
public class CoagulantRealtimeDAOImpl implements IProcessRealtimeDAO
{
    ...
    public int insert(List<ProcessRealtimeDTO> list)
    {
        int[] result = jdbcTemplate.batchUpdate(
                "INSERT IGNORE INTO coagulant_realtime VALUES(?, ?, ?, ?);",
    ...
    {
        String strQuery = String.format("SELECT * FROM coagulant_realtime PARTITION(p_%s) WHERE update_time IN " +
                "(SELECT MAX(update_time) FROM coagulant_realtime PARTITION(p_%s) GROUP BY name)", partitionName, partitionName);
        ...
        {
            strQuery = String.format("SELECT * FROM coagulant_realtime PARTITION(p_max) WHERE update_time IN " +
                    "(SELECT MAX(update_time) FROM coagulant_realtime PARTITION(p_max) GROUP BY name)");
    ...
    {
        String strQuery = "SELECT * FROM coagulant_realtime WHERE update_time > ? ORDER BY update_time DESC";
    ...
    {
        String strQuery = "SELECT * FROM coagulant_realtime WHERE name=? AND update_time > ? AND update_time < ?";
        return jdbcTemplate.query(
                strQuery,
    ...
    {
        String strQuery =  "SELECT * FROM coagulant_realtime WHERE name=? ORDER BY update_time DESC limit 1";
        try
        {
    ...
    public List<FrequencyDTO> selectDistribution(Date start_time, String name)
    {
        String strQuery = "select truncate(value, 2) as value, count(truncate(value, 2)) as count " +
                "FROM coagulant_realtime where update_time > ? and name = ? group by truncate(value, 2)";
    ...
    {
        String strQuery = String.format("ALTER TABLE coagulant_realtime REORGANIZE PARTITION p_max INTO (" +
    ...
    {
        String strQuery = String.format("ALTER TABLE coagulant_realtime DROP PARTITION p_%s", partitionName);
        jdbcTemplate.execute(strQuery);
    }
}
