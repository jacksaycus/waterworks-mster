...
public class AiOzoneAlarmDAOImpl implements IAiProcessAlarmDAO
{
    ...
    {
        String strQuery = "insert into ai_ozone_alarm values (?, ?, ?)";
    ...
    {
        String strQuery = "SELECT * FROM ai_ozone_alarm WHERE alarm_time>=? AND kafka_flag=? ORDER BY alarm_time DESC";
    ...
    {
        String strQuery = "UPDATE ai_ozone_alarm SET kafka_flag=? WHERE alarm_id=? AND alarm_time=?";
    ...
    {
        String strQuery = "DELETE FROM ai_ozone_alarm WHERE alarm_time<?";
