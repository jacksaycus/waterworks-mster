...
@Repository
public class FilterRealtimeDAOImpl implements IProcessRealtimeDAO
{
    @Autowired
    ...
    public int insert(List<ProcessRealtimeDTO> list)
    {
        int[] result = jdbcTemplate.batchUpdate(
                "INSERT IGNORE INTO filter_realtime VALUES(?, ?, ?, ?);",
    ...
    {
        String strQuery = String.format("SELECT * FROM filter_realtime PARTITION(p_%s) WHERE update_time IN " +
                "(SELECT MAX(update_time) FROM filter_realtime PARTITION(p_%s) GROUP BY name)", partitionName, partitionName);
        ...
        {
            strQuery = String.format("SELECT * FROM filter_realtime PARTITION(p_max) WHERE update_time IN " +
                    "(SELECT MAX(update_time) FROM filter_realtime PARTITION(p_max) GROUP BY name)");
    ...
    {
        String strQuery = "SELECT * FROM filter_realtime WHERE update_time > ? ORDER BY update_time DESC";
    ...
    {
        String strQuery = "SELECT * FROM filter_realtime WHERE name=? AND update_time > ? AND update_time < ?";
        return jdbcTemplate.query(
                strQuery,
    ...
    {
        String strQuery =  "SELECT * FROM filter_realtime WHERE name=? ORDER BY update_time DESC limit 1";
        try
        {
    ...
    {
        String strQuery = String.format("ALTER TABLE filter_realtime REORGANIZE PARTITION p_max INTO (" +
    ...
    {
        String strQuery = String.format("ALTER TABLE filter_realtime DROP PARTITION p_%s", partitionName);
        jdbcTemplate.execute(strQuery);
    }
}
