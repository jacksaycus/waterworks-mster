...
public class AiMixingAlarmDAOImpl implements IAiProcessAlarmDAO
{
    ...
    {
        String strQuery = "insert into ai_mixing_alarm values (?, ?, ?)";
    ...
    {
        String strQuery = "SELECT * FROM ai_mixing_alarm WHERE alarm_time>=? AND kafka_flag=? ORDER BY alarm_time DESC";
    ...
    {
        String strQuery = "UPDATE ai_mixing_alarm SET kafka_flag=? WHERE alarm_id=? AND alarm_time=?";
    ...
    {
        String strQuery = "DELETE FROM ai_mixing_alarm WHERE alarm_time<?";
